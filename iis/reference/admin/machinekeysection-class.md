...
|`DecryptionKey`|A read/write trimmed `string` value that either specifies the process by which the key is generated or contains text that is used to encrypt and decrypt data. The default is "AutoGenerate,IsolateApps". **Note:**  Use the "AutoGenerate" string to specify that [!INCLUDE[vstecasp](../../reference/includes/vstecasp-md.md)] generates a random key and stores it in the Local Security Authority. The "AutoGenerate" string is part of the default value. If you add the "IsolateApps" modifier to the `DecryptionKey` value, [!INCLUDE[vstecasp](../../reference/includes/vstecasp-md.md)] generates a unique encrypted key for each application by using its application ID. "IsolateApps" is also part of the default value. If you need to support configuration across a network of Web servers (a Web farm), set the `DecryptionKey` property manually to ensure consistent configuration.|  
